...
Decision tree regression is very simple in idea. We want to predict a response *y* given a multi-dimensional feature vector ***x***. To do that, we construct a binary tree where each non-leaf node contains two information: splitting dimension *k* and a corresponding splitting value *v<sub>k</sub>*. To make a prediction given ***x***, we traverse down the tree, starting from the root. At each node, if the splitting dimension of ***x*** is smaller than its splitting value, i.e. *x<sub>k</sub> < v<sub>k</sub>*, we proceed to the left child, otherwise we head right. This repeats until we hit a leaf node where there is no further split, and the node contains a value of the reponse variable, which will be taken as our prediction. The schemetics above shows a sample tree that predicts the amount of rainfall in millimeters based on 4 variables: humidity, temperature , wind speed and altitude.
